...
func BuildRequestObject(request Request) (*http.Request, error) {
	...
	if err != nil {
		err = mojoerror.New("EncodingError", "Error constructing http request", err)
		return req, err
	}
...
func BuildResponse(res *http.Response) (*Response, error) {
	...
	if err != nil {
		err := mojoerror.New("EncodingError", "Error reading the response body", err)
		return nil, err
	}

	if res.StatusCode < 200 || res.StatusCode > 299 {
		err = mojoerror.New("MojoAuthRespondedWithError", "Received error response from MojoAuth", errors.New(string(body)))
	} else {
		response = Response{
...
func (c *Client) Send(request Request) (*Response, error) {
	...
	if err != nil {
		err := mojoerror.New("MakeRequestError", "Error making the request", err)
		return nil, err
	}
...
func EncodeBody(body interface{}) (*bytes.Buffer, error) {
	...
	if ok {
		...
		if encodeErr != nil {
			err := mojoerror.New("EncodingError", "Error encoding the request body", encodeErr)
			return nil, err
		}
	} else {
...
func Validate(allowed map[string]bool, params interface{}) (map[string]string, error) {
	...
	if !ok {
		...
		err := mojoerror.New("ValidationError", "Error validating params - params type error", errors.New(errMsg))
		return nil, err
	}

	for k, _ := range asserted {
		if !allowed[k] {
			err := mojoerror.New("ValidationError", "Error validating params - invalid params submitted, please double check", errors.New("Error validating params"))
			return nil, err
		}
	}
	return asserted, nil
